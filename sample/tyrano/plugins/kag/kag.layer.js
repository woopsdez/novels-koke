tyrano.plugin.kag.layer = {
    ...
    init: function() {
        ...
        layer_obj_click.click(function(e) {
            doScroll();
            if ($(".menu_window_close").filter(".menu_window_closeOFF").size() != 0) {
                $(".menu_window_close").removeClass("menu_window_closeOFF").show()
                $(".menu_map").show();
                $(".menu_chara").show();
            }
            if (layer_menu.width() == 120 && layer_menu.css("display") == "block") {
                layer_menu.hide();
                $(".button_menu").show()
            }
        ...
        var layer_menu = $("<div class='layer layer_menu' style='z-index:100000000;display:none;right:0'  align='center'></div>");
        layer_menu.css("width", "120px").css("height", this.kag.config.scHeight).css("position", "absolute");
    ...
    refMessageLayer: function() {
        ...
        while (true) {
            if (this.map_layer_fore["message" + num]) {
                ...
                j_message_inner.css("left", parseInt(j_message_outer.css("left")) + 10).css("top", parseInt(j_message_outer.css("top")) + 10).css("width", parseInt(j_message_outer.css("width")) - 10).css("height", parseInt(j_message_outer.css("height")) - 10)
            } else break;
            num++
        }
    },
