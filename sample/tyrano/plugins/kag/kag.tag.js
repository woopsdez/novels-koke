tyrano.plugin.kag.ftag = {
    ...
    nextOrder: function () {
        ...
        try {
            ...
            if (this.master_tag[tag.name]) {
                ...
                $.error_message("\u30bf\u30b0\uff1a[" + tag.name + "]\u306f\u5b58\u5728\u3057\u307e\u305b\u3093");
                this.nextOrder()
            }
        } catch (e) {
            console.log(e);
    ...
    convertEntity: function (pm) {
        var that = this;
        if (pm["*"] == "") pm = $.extend(true, $.cloneObject(pm), this.kag.stat.mp);
        for (key in pm) {
            var val = pm[key];
            var c = "";
            ...
            if (val.length > 0 && c === "&") pm[key] = this.kag.embScript(val.substr(1, val.length));
                ...
                var d = val.split("|");
                if (d.length == 2)
                    if (map_obj.pm[$.trim(d[0]).substr(1, $.trim(d[0]).length)]) pm[key] =
    ...
    checkVital: function (tag) {
        ...
        for (var i = 0; i < array_vital.length; i++)
            if (tag.pm[array_vital[i]]) {
                if (tag.pm[array_vital[i]] == "") err_str += "\u30bf\u30b0\u300c" + tag.name + "\u300d\u306b\u30d1\u30e9\u30e1\u30fc\u30bf\u30fc\u300c" + array_vital[i] + "\u300d\u306f\u5fc5\u9808\u3067\u3059\u3000\n"
            } else err_str +=
    ...
    nextOrderWithIndex: function (index, scenario_file, flag, insert, auto_next) {
        ...
        if (auto_next ==
            ...
            if (this.kag.stat.is_skip == true && this.kag.stat.is_strong_stop == false) this.kag.ftag.nextOrder()
        } else if (auto_next == "stop") this.kag.stat.is_strong_stop = true
    }
};
tyrano.plugin.kag.tag.text = {
    ...
    start: function (pm) {
        ...
        j_inner_message.css("letter-spacing", this.kag.config.defaultPitch + "px").css("line-height", parseInt(this.kag.config.defaultFontSize) + parseInt(this.kag.config.defaultLineSpacing) +
            "px").css("font-family", this.kag.config.userFace);
...
tyrano.plugin.kag.tag.position = {
    ...
    start: function (pm) {
        ...
        if (pm.vertical == "true") this.kag.stat.vertical = "true";
        else this.kag.stat.vertical =
            "false"; if (pm.frame == "none") {
        ...
        if (pm.marginb != "0") new_style_inner["height"] = parseInt(layer_inner.css("height")) -
            parseInt(pm.marginb) + "px";
...
tyrano.plugin.kag.tag.image = {
    ...
    start: function (pm) {
        ...
        if (pm.layer != "base") {
            var layer_new_style = {};
            if (pm.visible == "true" && pm.page == "fore") layer_new_style.display = "block";
            ...
            if (pm.width != "") img_obj.css("width", pm.width + "px");
            if (pm.height != "") img_obj.css("height", pm.height + "px");
            if (pm.x != "") img_obj.css("left", pm.x + "px");
            if (pm.y != "") img_obj.css("top", pm.y + "px");
            ...
            if (pm.page === "fore") new_style.display = "block";
...
tyrano.plugin.kag.tag.freeimage = {
    ...
    start: function (pm) {
        if (pm.layer != "base") this.kag.layer.getLayer(pm.layer, pm.page).empty();
        else this.kag.layer.getLayer(pm.layer, pm.page).css("background-image", "");
        this.kag.ftag.nextOrder()
    }
};
...
tyrano.plugin.kag.tag.quake = {
    ...
    start: function (pm) {
        var that = this;
        if (pm.hmax != null) $("." + this.kag.define.BASE_DIV_NAME).effect("shake", {
...
tyrano.plugin.kag.tag.button = {
    ...
    start: function (pm) {
        ...
        else j_button.css("left", pm.x + "px"); if (pm.y == "") j_button.css("top", this.kag.stat.locate.y + "px");
        else j_button.css("top", pm.y + "px"); if (pm.fix != "false") j_button.addClass("fixlayer");
        ...
        (function () {
            ...
            j_button.click(function () {
                if (button_clicked == true) return false;
